...
Page({
  ...
  uploadCanvasImg() {
    ...
    wx.canvasToTempFilePath({
      ...
      success(res1) {
        ...
        //上传
        wx.uploadFile({
          url: `${address.HTTP_BASE_URL}/personaluser/save`,
          header: {
          ...
          success(res) {
            ...
            if (data.code == "2000" && data.data) {
              ...
              wx.showToast({
                title: data.message ? data.message : '上传失败,请稍后再试',
                icon: "none",
                duration: 3000,
              });
            }
  ...
  onLoad: function (options) {
    ...
    // let ctx = null;
    //  ctx = wx.createCanvasContext(canvasName); 废弃


    wx.createSelectorQuery()
      .select(`#${canvasName}`) // 在 WXML 中填入的 id
      .fields({
        node: true,
        size: true
      })
